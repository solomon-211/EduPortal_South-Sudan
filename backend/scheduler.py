...
from notification import already_notified, create_notification, send_email
...
def start(app) -> BackgroundScheduler:
    ...
    def _backup_job():
        with app.app_context():
            try:
                dest = run_backup()
                file_size = os.path.getsize(dest) if dest else None
                execute(
                    "INSERT INTO backup_runs (success,file_path,file_size,message) VALUES (?,?,?,?)",
                    (1, dest or None, file_size, None),
                )
            except Exception as exc:
                log.exception("Scheduled job run_backup failed")
                execute(
                    "INSERT INTO backup_runs (success,file_path,file_size,message) VALUES (?,?,?,?)",
                    (0, None, None, str(exc)[:500]),
                )
