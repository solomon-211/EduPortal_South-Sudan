"""Everything database: engine/connection, query helpers, and first-run setup.

Was three files (db_connection.py, db_queries.py, db_schema.py) that always
got touched together — merged into one so the whole data-access layer reads
top to bottom instead of jumping files.
"""
...
# ── Connection ──────────────────────────────────────────────────────────
# One pooled engine for the process; get_conn() reuses a single connection
# per request via Flask's `g`, get_db() is the standalone context-manager
# version for background jobs and scripts that run outside a request.
...
# ── Queries ─────────────────────────────────────────────────────────────
# Every call site writes SQL with `?` placeholders (sqlite3-style) since
# that's what the rest of the codebase was written against; _bind() converts
# them to SQLAlchemy's `:name` style before execution.
...
# ── First-run setup ─────────────────────────────────────────────────────
# init_db() is what app.py calls on startup: apply migrations, then make
# sure there's an admin account to actually log in with.
