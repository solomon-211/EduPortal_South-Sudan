...
from database import engine  # noqa: E402 — must come after env setup
...
def _apply_schema() -> None:
    ...
    ddl = """
        ...
        CREATE TABLE IF NOT EXISTS schools (
            ...
            teaching_staff_count INTEGER,
            classroom_count INTEGER,
            has_library INTEGER NOT NULL DEFAULT 0,
            has_laboratory INTEGER NOT NULL DEFAULT 0,
            has_sports_facilities INTEGER NOT NULL DEFAULT 0,
            has_water_sanitation INTEGER NOT NULL DEFAULT 0,
            has_electricity INTEGER NOT NULL DEFAULT 0,
            has_sports_clubs INTEGER NOT NULL DEFAULT 0,
            has_arts_culture INTEGER NOT NULL DEFAULT 0,
            has_academic_clubs INTEGER NOT NULL DEFAULT 0,
            has_student_government INTEGER NOT NULL DEFAULT 0,
...
# Patch init_db to skip Alembic in tests
import database as _database_module
_database_module.init_db = lambda: None
...
class TestMaterials:
    ...
    def test_download_forbidden_for_parent(self, client):
        _register(client, "parent_dl@test.ss", role="parent")
        token = _login(client, "parent_dl@test.ss")
        rv = client.get("/api/materials/1/download", headers=_auth(token))
        assert rv.status_code == 403
...
class TestBookmarks:
    ...
    def test_save_bookmark_forbidden_for_parent(self, client):
        _register(client, "bm_parent@test.ss", role="parent")
        token = _login(client, "bm_parent@test.ss")
        rv = client.post("/api/bookmarks", json={"item_type": "school", "item_id": 1},
                         headers=_auth(token))
        assert rv.status_code == 403
...
class TestScholarshipMedia:
    def _make_scholarship(self) -> int:
        with engine.connect() as conn:
            sid = conn.execute(text(
                "INSERT INTO scholarships (title,description,eligibility,deadline,how_to_apply,approved) "
                "VALUES ('Media Test Scholarship','desc','elig','2099-01-01','apply',1)"
            )).lastrowid
            conn.commit()
        return sid

    def _admin_token(self, client, email: str) -> str:
    ...
    def test_poster_upload_requires_ngo_or_admin_role(self, client):
        _register(client, "student_poster@test.ss", role="student")
        token = _login(client, "student_poster@test.ss")
        sid = self._make_scholarship()
        rv = client.post(
            f"/api/scholarships/{sid}/poster",
            data={"poster": (io.BytesIO(b"\x89PNG\r\n\x1a\n" + b"\x00" * 50), "poster.png")},
            content_type="multipart/form-data", headers=_auth(token),
        )
        assert rv.status_code == 403

    def test_poster_upload_not_found(self, client):
        token = self._admin_token(client, "admin_poster1@test.ss")
        rv = client.post(
            "/api/scholarships/999999/poster",
            data={"poster": (io.BytesIO(b"\x89PNG\r\n\x1a\n" + b"\x00" * 50), "poster.png")},
            content_type="multipart/form-data", headers=_auth(token),
        )
        assert rv.status_code == 404

    def test_poster_upload_rejects_fake_image(self, client):
        token = self._admin_token(client, "admin_poster2@test.ss")
        sid = self._make_scholarship()
        rv = client.post(
            f"/api/scholarships/{sid}/poster",
            data={"poster": (io.BytesIO(b"not an image"), "poster.png")},
            content_type="multipart/form-data", headers=_auth(token),
        )
        assert rv.status_code == 400

    def test_poster_upload_accepts_valid_image(self, client):
        token = self._admin_token(client, "admin_poster3@test.ss")
        sid = self._make_scholarship()
        rv = client.post(
            f"/api/scholarships/{sid}/poster",
            data={"poster": (io.BytesIO(b"\x89PNG\r\n\x1a\n" + b"\x00" * 50), "poster.png")},
    ...
    def test_video_upload_requires_ngo_or_admin_role(self, client):
        _register(client, "student_promo@test.ss", role="student")
        token = _login(client, "student_promo@test.ss")
        sid = self._make_scholarship()
        rv = client.post(
            f"/api/scholarships/{sid}/video",
            data={"video": (io.BytesIO(b"\x1aE\xdf\xa3" + b"\x00" * 50), "promo.webm")},
            content_type="multipart/form-data", headers=_auth(token),
        )
        assert rv.status_code == 403

    def test_video_upload_rejects_wrong_extension(self, client):
        token = self._admin_token(client, "admin_promo1@test.ss")
        sid = self._make_scholarship()
        rv = client.post(
            f"/api/scholarships/{sid}/video",
            data={"video": (io.BytesIO(b"data"), "notes.txt")},
            content_type="multipart/form-data", headers=_auth(token),
        )
        assert rv.status_code == 400

    def test_video_upload_rejects_fake_video(self, client):
        token = self._admin_token(client, "admin_promo2@test.ss")
        sid = self._make_scholarship()
        rv = client.post(
            f"/api/scholarships/{sid}/video",
            data={"video": (io.BytesIO(b"not a real video"), "promo.mp4")},
            content_type="multipart/form-data", headers=_auth(token),
        )
        assert rv.status_code == 400

    def test_video_upload_accepts_valid_webm_magic(self, client):
        token = self._admin_token(client, "admin_promo3@test.ss")
        sid = self._make_scholarship()
        rv = client.post(
            f"/api/scholarships/{sid}/video",
            data={"video": (io.BytesIO(b"\x1aE\xdf\xa3" + b"\x00" * 50), "promo.webm")},
...
class TestGoogleAuth:
    ...
    def _fake_claims(self, monkeypatch, email="google.newuser@test.ss"):
        import app as app_module
        monkeypatch.setattr(app_module, "verify_google_token", lambda credential: {
            "email": email, "name": "Google New User", "email_verified": True, "picture": None,
        })

    def test_new_account_needs_role_first(self, client, monkeypatch):
        self._fake_claims(monkeypatch)
        rv = client.post("/api/auth/google", json={"credential": "fake"})
        data = rv.get_json()
        assert rv.status_code == 200
        assert data["needs_role"] is True
        assert "token" not in data  # no account, no session, until a role is chosen

    def test_new_account_cannot_self_assign_admin(self, client, monkeypatch):
        self._fake_claims(monkeypatch)
        rv = client.post("/api/auth/google", json={"credential": "fake", "role": "admin"})
        data = rv.get_json()
        assert data["needs_role"] is True  # rejected — falls back to asking again, never creates the account

    def test_new_account_created_with_chosen_role(self, client, monkeypatch):
        self._fake_claims(monkeypatch, email="google.ngo@test.ss")
        rv = client.post("/api/auth/google", json={"credential": "fake", "role": "ngo_officer"})
        data = rv.get_json()
        assert rv.status_code == 200
        assert data["token"]
        assert data["user"]["role"] == "ngo_officer"

    def test_google_only_account_rejects_password_login(self, client, monkeypatch):
        # A Google-signup account never got a real password — the random
        # placeholder hash means credential login must fail with a message
        # pointing at Google sign-in / reset-password, not a bare 401.
        self._fake_claims(monkeypatch, email="google.nopass@test.ss")
        rv = client.post("/api/auth/google", json={"credential": "fake", "role": "student"})
        assert rv.status_code == 200
        rv2 = client.post("/api/login", json={
            "identifier": "google.nopass@test.ss", "password": "WhateverGuess1!",
        })
        assert rv2.status_code == 401
        assert "google" in rv2.get_json()["error"].lower()

    def test_google_only_account_can_set_password_via_reset(self, client, monkeypatch):
        self._fake_claims(monkeypatch, email="google.reset@test.ss")
        rv = client.post("/api/auth/google", json={"credential": "fake", "role": "student"})
        user_id = rv.get_json()["user"]["id"]
        rv2 = client.post("/api/forgot-password", json={"identifier": "google.reset@test.ss"})
        token = rv2.get_json()["dev_token"]
        rv3 = client.post("/api/reset-password", json={
            "user_id": user_id, "token": token, "new_password": "NewRealPass1!",
        })
        assert rv3.status_code == 200
        rv4 = client.post("/api/login", json={
            "identifier": "google.reset@test.ss", "password": "NewRealPass1!",
        })
        assert rv4.status_code == 200
        assert rv4.get_json()["token"]
