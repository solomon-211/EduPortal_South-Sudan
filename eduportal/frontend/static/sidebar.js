(function () {
  ...
  // Brand
  html += '<a class="sidebar-brand" href="/dashboard">';
  html += '<div class="sidebar-brand-icon">' + LOGO_SVG + '</div>';
  html += '<div><strong>EduPortal</strong><span>South Sudan</span></div>';
  html += '</a>';

  // Account chip
  if (user) {
    html += '<div class="sidebar-account">';
    html += '<div class="sidebar-avatar">' + esc(initials) + '</div>';
    html += '<div style="min-width:0">';
    html += '<strong style="display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + esc(user.name) + '</strong>';
