(function () {
  ...
  function downloadChecklistPDF(school, items) {
    ...
    const rows = items.map(r => {
      ...
      const notes = r.notes ? `<br><small style="color:#666">${r.notes}</small>` : '';
      return `<tr><td style="width:32px;font-size:1.1rem;vertical-align:top;padding:6px 8px">${checked}</td><td style="padding:6px 8px;vertical-align:top"><strong>${r.item_label}</strong>${optional}${notes}</td></tr>`;
    }).join('');
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8">
      ...
      <table>${rows}</table>
      <p style="margin-top:1.5rem;font-size:0.78rem;color:#aaa">Printed from EduPortal South Sudan &middot; eduportal.ss</p>
    </body></html>`;
  ...
  function appStatusBadge(status) {
    ...
    return `<span style="display:inline-block;padding:0.2rem 0.65rem;border-radius:999px;font-size:0.75rem;font-weight:800;background:${color}18;color:${color}">${labels[status] || esc(status)}</span>`;
  }
  ...
  function renderPagination(container, total, page, perPage, onPage) {
    ...
    const items = [];
    items.push(`<button class="pag-btn" data-page="${page-1}" ${page===1?'disabled':''}>← Prev</button>`);
    ...
    try {
      ...
      if (container) {
        container.innerHTML = items.slice(0, 3).map(s => `
          ...
          </div>`).join('') || '<p style="color:#888">No scholarships available.</p>';
      }
    } catch (err) {
      const container = document.getElementById('dashboard-scholarships');
      if (container) {
  ...
  async function initDirectory() {
      ...
      try {
        ...
        inlineDetail.innerHTML = `
          <div style="padding:0.25rem">
            ...
            <h2 style="margin:0 0 0.3rem;font-size:1.25rem;color:var(--maroon-dark)">${esc(school.name)}</h2>
            <p style="margin:0 0 0.5rem;color:#888;font-size:0.88rem">${esc(school.county)} &middot; ${esc(school.level)} &middot; ${esc(school.boarding || 'Day')} &middot; ${esc(school.type || 'Mixed')}</p>
            <p style="margin:0 0 0.4rem;font-size:0.88rem">${esc(school.description || '')}</p>
            <p style="margin:0 0 0.3rem;font-size:0.84rem;color:#888">Hours: ${esc(school.hours || 'N/A')} &middot; Capacity: ${esc(school.capacity)} &middot; Enrolled: ${esc(school.enrollment)}</p>
            <p style="margin:0 0 0.8rem;font-size:0.84rem;color:#888">Contact: ${esc(school.contact_name)} &middot; ${esc(school.phone)}</p>
            <div style="display:flex;gap:0.5rem;flex-wrap:wrap">
              <a class="card-button" href="/schools/${school.id}" style="width:auto;padding:0.55rem 1rem;font-size:0.85rem;margin-top:0">Full profile</a>
              ${getToken() ? `<button class="card-button bookmark-btn" data-type="school" data-id="${school.id}" style="width:auto;padding:0.55rem 1rem;font-size:0.85rem;margin-top:0;background:transparent;color:var(--maroon);border:1.5px solid rgba(139,26,26,0.2);box-shadow:none">Save school</button>` : ''}
            </div>
          </div>`;
        if (inlineReqs) {
          inlineReqs.innerHTML = `<p class="section-label" style="margin:0 0 0.6rem">ADMISSION CHECKLIST</p>` + (items.length
            ? items.map(r => `
                <label class="checklist-item">
                  <input type="checkbox" ${r.is_required ? 'checked' : ''} disabled>
                  <span>
                    <strong>${esc(r.item_label)}</strong>
                    ${!r.is_required ? '<span class="tag tag-muted" style="font-size:0.72rem;margin-left:0.4rem">Optional</span>' : ''}
                    ${r.notes ? `<span style="display:block;color:#888;font-size:0.84rem">${esc(r.notes)}</span>` : ''}
                  </span>
                </label>`).join('')
            : '<p style="color:#888;font-size:0.88rem">No requirements listed yet.</p>');
        }
        ...
        inlineDetail.innerHTML = `<p style="color:#c0392b">${esc(err.message)}</p>`;
      }
    }
    ...
    async function load(page = 1) {
      ...
      try {
        ...
        results.innerHTML = `<p style="color:#c0392b">${esc(err.message)}</p>`;
      }
    }
  ...
  async function initSchoolProfile() {
    ...
    if (!id) { shell.innerHTML = '<p style="color:#c0392b">School not found.</p>'; return; }
    ...
    try {
      ...
      if (detail) {
        detail.innerHTML = `
          ...
          <h1 style="margin:0 0 0.4rem;font-size:clamp(1.5rem,3vw,2.2rem);color:var(--maroon-dark)">${esc(school.name)}</h1>
          <p style="margin:0 0 0.8rem;color:#888">${esc(school.county)}, ${esc(school.state)} &middot; ${esc(school.level)} &middot; ${esc(school.boarding || 'Day')}</p>
          <p style="margin:0 0 1rem">${esc(school.description || '')}</p>
          ...
          ${getToken() ? `<button class="card-button bookmark-btn" data-type="school" data-id="${school.id}" style="width:auto;padding:0.65rem 1.4rem;margin-top:0">Save school</button>` : ''}`;
      }
      ...
      if (checklist) {
          ...
          ? items.map(r => `
              <label class="checklist-item">
                ...
                <span>
                  <strong>${esc(r.item_label)}</strong>
                  ${!r.is_required ? '<span class="tag tag-muted" style="font-size:0.72rem;margin-left:0.4rem">Optional</span>' : ''}
                  ${r.notes ? `<span style="display:block;color:#888;font-size:0.88rem;margin-top:0.2rem">${esc(r.notes)}</span>` : ''}
                </span>
              </label>`).join('')
        ...
        if (items.length) {
          ...
          dlBtn.className = 'card-button';
          dlBtn.style.cssText = 'width:auto;padding:0.65rem 1.4rem;margin-top:1rem;display:inline-flex;gap:0.5rem;align-items:center';
  ...
  function _renderSchoolEditPanel(shell, school, reqItems, id) {
    ...
    panel.innerHTML = `
      ...
      <p class="school-note" style="margin-bottom:1rem">You are the assigned admin for this school. Changes are saved immediately.</p>

      <form id="edit-school-form">
        <div class="form-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:0.75rem 1rem;margin-bottom:1rem">
        ...
        <label class="field-label" style="display:block;margin-bottom:1rem">Description
          <textarea class="field-input" name="description" rows="3" style="resize:vertical">${esc(school.description||'')}</textarea>
        </label>
        <button class="card-button" type="submit" style="width:auto;padding:0.65rem 1.4rem;margin-top:0">Save School Info</button>
        <span id="edit-school-msg" style="margin-left:0.8rem;font-size:0.88rem"></span>
      </form>

      <hr style="margin:1.4rem 0;border:0;border-top:1px solid rgba(91,15,15,0.1)">

      <p class="section-label" style="margin-bottom:0.6rem">ADMISSION REQUIREMENTS</p>
      <div id="req-editor"></div>
      <button id="add-req-btn" class="card-button" style="width:auto;padding:0.55rem 1rem;margin-top:0.6rem;background:transparent;color:var(--maroon);border:1.5px solid rgba(139,26,26,0.2);box-shadow:none">+ Add requirement</button>
      <button id="save-reqs-btn" class="card-button" style="width:auto;padding:0.65rem 1.4rem;margin-top:0.6rem;margin-left:0.5rem">Save Requirements</button>
      <span id="edit-reqs-msg" style="margin-left:0.8rem;font-size:0.88rem"></span>`;
    ...
    function renderReqEditor() {
      reqEditor.innerHTML = reqs.map((r, i) => `
        <div class="req-row" style="display:flex;gap:0.5rem;align-items:flex-start;margin-bottom:0.5rem" data-idx="${i}">
          <input class="field-input req-label" placeholder="Requirement label" value="${esc(r.item_label)}" style="flex:2">
          <input class="field-input req-notes" placeholder="Notes (optional)" value="${esc(r.notes)}" style="flex:2">
          <label style="display:flex;align-items:center;gap:0.3rem;font-size:0.82rem;white-space:nowrap;padding-top:0.6rem">
            <input type="checkbox" class="req-required" ${r.is_required ? 'checked' : ''}> Required
          </label>
          <button class="req-remove" data-idx="${i}" style="padding:0.45rem 0.7rem;border:1.5px solid rgba(192,57,43,0.3);border-radius:8px;background:transparent;color:#c0392b;cursor:pointer;font-size:0.82rem;flex-shrink:0;margin-top:0.1rem">&times;</button>
        </div>`).join('');
  ...
  async function initMaterials() {
      ...
      if (!uploadForm.querySelector('[name="file"]')) {
        const fileFieldHTML = `
          <div class="field" style="grid-column:1/-1">
            ...
            <input type="file" name="file" accept=".pdf" style="border:1.5px solid rgba(91,15,15,0.13);border-radius:10px;padding:0.6rem 0.9rem;background:#fdfaf9;width:100%;cursor:pointer">
          </div>`;
    ...
    async function load(page) {
      ...
      try {
        ...
        results.innerHTML = data.items.length
          ? data.items.map(m => `
              <article class="result-card material-card" data-id="${m.id}" tabindex="0" role="button" style="cursor:pointer">
            ...
            if (m && inlineDetail) {
              inlineDetail.innerHTML = `
                <div style="padding:0.25rem">
                  ...
                  <h2 style="margin:0 0 0.3rem;font-size:1.15rem;color:var(--maroon-dark)">${esc(m.title)}</h2>
                  <p style="margin:0 0 0.5rem;color:#888;font-size:0.85rem">${esc(m.year)} &middot; ${esc(m.file_size || 'N/A')}</p>
                  <p style="margin:0 0 0.8rem;font-size:0.9rem">${esc(m.preview_text || '')}</p>
                  <div style="display:flex;gap:0.5rem;flex-wrap:wrap">
                    ${getToken() ? `<a class="card-button" href="/api/materials/${m.id}/download" style="width:auto;padding:0.55rem 1rem;font-size:0.85rem;margin-top:0" download>
                        <svg viewBox="0 0 16 16" fill="none" width="13" height="13" style="margin-right:0.3rem"><path d="M8 2v8M5 7l3 3 3-3" stroke="white" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/><path d="M2 12h12" stroke="white" stroke-width="1.4" stroke-linecap="round"/></svg>
                        Download PDF
                      </a>` : `<a class="card-button" href="/login" style="width:auto;padding:0.55rem 1rem;font-size:0.85rem;margin-top:0">Login to download</a>`}
                    ${getToken() ? `<button class="card-button detail-bookmark-btn" data-id="${m.id}" style="width:auto;padding:0.55rem 1rem;font-size:0.85rem;margin-top:0;background:transparent;color:var(--maroon);border:1.5px solid rgba(139,26,26,0.2);box-shadow:none">Save material</button>` : ''}
                  </div>
                </div>`;
      ...
      detailContent.innerHTML = `
        <div class="detail-header">
          ...
          <div style="display:flex;gap:0.6rem;flex-wrap:wrap;flex-shrink:0">
            ${getToken() ? `<button class="card-button apply-btn" data-id="${s.id}" style="width:auto;padding:0.65rem 1.4rem;margin-top:0">Apply Now</button>
            <button class="card-button sch-bookmark-btn" data-id="${s.id}" style="width:auto;padding:0.65rem 1.4rem;margin-top:0;background:transparent;color:var(--maroon);border:1.5px solid rgba(139,26,26,0.2);box-shadow:none">&#9825; Save</button>` : '<a class="card-button" href="/login" style="width:auto;padding:0.65rem 1.4rem;margin-top:0">Login to Apply</a>'}
          </div>
        </div>
        <div class="detail-body">
          ...
          ${s.external_link ? `<div class="detail-section"><a href="${esc(s.external_link)}" target="_blank" rel="noopener" class="card-button" style="width:auto;padding:0.65rem 1.4rem;margin-top:0;display:inline-flex">Visit Application Page &rarr;</a></div>` : ''}
    ...
    async function load() {
      ...
      try {
        ...
        results.innerHTML = items.length
          ? items.map(s => `
              <article class="result-card opportunity-card" data-id="${s.id}" style="cursor:pointer" tabindex="0" role="button">
                ...
                <div class="result-card-footer">
                  <span class="deadline-badge">&#128197; ${esc(s.deadline)}</span>
              ...
              <article class="ann-list-item">
                ...
                <div class="ann-list-body">
                  ...
                  <div class="ann-list-footer">
                    ...
                    <span style="font-size:0.78rem;color:#aaa">${esc(a.created_at ? a.created_at.slice(0,10) : '')}</span>
                  </div>
                </div>
              </article>`).join('')
  ...
  async function initMyApplications() {
    ...
    async function load() {
      ...
      try {
            ...
            html += `
              <div class="app-card">
                ...
                <div style="display:flex;flex-direction:column;align-items:flex-end;gap:0.5rem;flex-shrink:0">
                  ...
                  ${a.status === 'submitted' ? `<button class="card-link withdraw-btn" data-id="${a.id}" style="border:0;background:none;cursor:pointer;font:inherit;font-size:0.78rem;color:#c0392b;padding:0">Withdraw</button>` : ''}
                </div>
              </div>`;
          });
        });
        ...
        listEl.innerHTML = `<p class="apps-empty" style="color:#c0392b">${esc(err.message)}</p>`;
      }
    }
  ...
  async function initAdmin() {
      ...
      el.innerHTML = items.length
        ? items.map(item => `
            <div class="admin-approve-card">
              <div>
                <p class="eyebrow" style="margin:0 0 0.3rem;color:var(--maroon)">${esc(type)}</p>
        ...
        : '<p style="color:#888">No items pending.</p>';
          ...
          el.innerHTML = items.length
            ? items.map(u => {
                ...
                const assignControl = u.role === 'school_admin' ? `
                  <select class="school-assign-select" data-id="${u.id}" style="padding:0.35rem 0.6rem;border:1.5px solid rgba(91,15,15,0.14);border-radius:8px;font:inherit;font-size:0.82rem;background:#fff;color:var(--ink);cursor:pointer;max-width:180px">
                    <option value="">No school assigned</option>
                    ${schoolOptions}
                  </select>
                  <button class="btn-assign-school" data-id="${u.id}" style="padding:0.35rem 0.75rem;border:1.5px solid rgba(91,15,15,0.2);border-radius:8px;background:transparent;color:var(--maroon);font:inherit;font-size:0.82rem;font-weight:700;cursor:pointer">Assign</button>` : '';
                return `
                <div class="admin-user-row">
                  <div>
                    <strong>${esc(u.name)}</strong>
                    <span>${esc(u.role)} &middot; ${esc(u.state)} &middot; ${esc(u.email || u.phone || 'no contact')}</span>
                    ${u.role === 'school_admin' && u.school_id ? `<span style="font-size:0.78rem;color:var(--maroon);display:block;margin-top:0.15rem">&#127eb; Assigned to school #${u.school_id}</span>` : ''}
                  </div>
                  <div style="display:flex;gap:0.5rem;align-items:center;flex-wrap:wrap">
                    <select class="role-select" data-id="${u.id}" style="padding:0.35rem 0.6rem;border:1.5px solid rgba(91,15,15,0.14);border-radius:8px;font:inherit;font-size:0.82rem;background:#fff;color:var(--ink);cursor:pointer">
                    ...
                    <button class="btn-change-role" data-id="${u.id}" style="padding:0.35rem 0.75rem;border:1.5px solid rgba(91,15,15,0.2);border-radius:8px;background:transparent;color:var(--maroon);font:inherit;font-size:0.82rem;font-weight:700;cursor:pointer">Set Role</button>
                    ${assignControl}
                    <button class="btn-suspend" data-id="${u.id}" ${u.verified === -1 ? 'disabled' : ''}>${u.verified === -1 ? 'Suspended' : 'Suspend'}</button>
                    <button class="btn-delete-user" data-id="${u.id}" style="padding:0.45rem 0.9rem;border:1.5px solid rgba(192,57,43,0.3);border-radius:999px;background:transparent;color:#c0392b;font:inherit;font-size:0.82rem;font-weight:700;cursor:pointer;white-space:nowrap;transition:background 150ms">Delete</button>
                  </div>
                </div>`;
              }).join('')
    ...
    async function loadAdminApps() {
      ...
      try {
        ...
        if (!items.length) { listEl.innerHTML = '<p style="color:#888">No applications found.</p>'; return; }
        listEl.innerHTML = items.map(a => `
          <div class="admin-approve-card" style="align-items:center">
            <div style="flex:1;min-width:0">
              <p class="eyebrow" style="margin:0 0 0.2rem;color:var(--maroon)">${esc(a.scholarship_title || 'Scholarship')}</p>
              <h3 style="margin:0 0 0.15rem;font-size:0.95rem">${esc(a.applicant_name || 'User #' + a.user_id)}</h3>
              <p style="margin:0;font-size:0.82rem;color:#888">${esc(a.applicant_email || '')} &middot; Applied ${esc(a.applied_at ? a.applied_at.slice(0,10) : '')}</p>
            </div>
            <div style="display:flex;align-items:center;gap:0.6rem;flex-shrink:0">
              ${appStatusBadge(a.status)}
              <select class="app-status-select" data-app-id="${a.id}" style="border:1.5px solid rgba(91,15,15,0.13);border-radius:8px;padding:0.4rem 0.65rem;font:inherit;font-size:0.82rem;background:#fdfaf9;color:var(--ink);cursor:pointer">
        ...
        listEl.innerHTML = `<p style="color:#c0392b">${esc(err.message)}</p>`;
      }
    }
          ...
          ? schools.map(s => `
              <div class="admin-user-row">
                <div>
                  ...
                  <span style="font-size:0.78rem;color:#888">${esc(s.boarding || 'Day')} &middot; Enrolled: ${esc(s.enrollment)}</span>
                </div>
                <div style="display:flex;gap:0.5rem;align-items:center;flex-wrap:wrap">
                  <a class="card-button" href="/schools/${s.id}" style="width:auto;padding:0.35rem 0.85rem;margin-top:0;font-size:0.82rem" target="_blank">View</a>
                  <button class="btn-delete-school" data-id="${s.id}" style="padding:0.35rem 0.85rem;border:1.5px solid rgba(192,57,43,0.3);border-radius:999px;background:transparent;color:#c0392b;font:inherit;font-size:0.82rem;font-weight:700;cursor:pointer">Delete</button>
                </div>
              </div>`).join('')
          : '<p style="color:#888">No schools found.</p>';
  ...
  async function initBookmarks() {
    ...
    try {
      ...
      ['bm-schools-list','bm-materials-list','bm-scholarships-list'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.innerHTML = `<p style="color:#c0392b">${esc(err.message)}</p>`;
      });
      return;
    }
    ...
    tabBtns.forEach(btn => {
      ...
      if (cnt !== undefined) {
        btn.innerHTML = btn.textContent.trim().split(' ')[0] + (cnt > 0 ? ` <span style="display:inline-block;background:var(--maroon);color:#fff;border-radius:999px;font-size:0.7rem;padding:0.1rem 0.45rem;margin-left:0.3rem;font-weight:800">${cnt}</span>` : '');
      }
    });
    ...
    function renderSchools() {
      ...
      el.innerHTML = bms.map(b => {
        const s = b.detail;
        if (!s) return `<article class="result-card"><p class="muted">School #${b.item_id} not found</p><div class="result-card-footer"><button class="card-link remove-bm" data-bm-id="${b.bookmark_id}" style="border:0;background:none;cursor:pointer;font:inherit;color:#c0392b;padding:0">Remove</button></div></article>`;
        return `<article class="result-card">
          ...
          <p class="result-card-preview">${esc(s.description || '')}</p>
          <div class="result-card-footer">
            <a class="card-link" href="/schools/${s.id}">View profile &rarr;</a>
            <button class="card-link remove-bm" data-bm-id="${b.bookmark_id}" style="border:0;background:none;cursor:pointer;font:inherit;color:#c0392b;padding:0;font-size:0.82rem">Remove</button>
          </div>
        </article>`;
      }).join('');
    ...
    function renderMaterials() {
      ...
      el.innerHTML = bms.map(b => {
        const m = b.detail;
        if (!m) return `<article class="result-card"><p class="muted">Material #${b.item_id} not found</p><div class="result-card-footer"><button class="card-link remove-bm" data-bm-id="${b.bookmark_id}" style="border:0;background:none;cursor:pointer;font:inherit;color:#c0392b;padding:0">Remove</button></div></article>`;
        return `<article class="result-card">
          <div class="result-card-top">
            <span class="tag">${esc(m.subject)}</span>
            <span class="tag tag-muted">${esc(m.grade)}</span>
          ...
          <div class="result-card-footer">
            ${getToken() && m.file_path
              ? `<a class="card-link" href="/api/materials/${m.id}/download" download style="color:var(--maroon)">
                  <svg viewBox="0 0 14 14" fill="none" width="12" height="12" style="vertical-align:middle;margin-right:2px"><path d="M7 1v7M4 6l3 3 3-3" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/><path d="M1 11h12" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg>
                  Download PDF
                </a>`
              : `<span class="card-link" style="color:#aaa">${m.file_path ? 'Login to download' : 'No file yet'}</span>`}
            <button class="card-link remove-bm" data-bm-id="${b.bookmark_id}" style="border:0;background:none;cursor:pointer;font:inherit;color:#c0392b;padding:0;font-size:0.82rem">Remove</button>
          </div>
        </article>`;
      }).join('');
    ...
    function renderScholarships() {
      ...
      el.innerHTML = bms.map(b => {
        const s = b.detail;
        if (!s) return `<article class="result-card"><p class="muted">Scholarship #${b.item_id} not found</p><div class="result-card-footer"><button class="card-link remove-bm" data-bm-id="${b.bookmark_id}" style="border:0;background:none;cursor:pointer;font:inherit;color:#c0392b;padding:0">Remove</button></div></article>`;
        return `<article class="result-card">
          <div class="result-card-top"><span class="tag">Scholarship</span></div>
          <h3 class="result-card-title">${esc(s.title)}</h3>
          <p class="org">${esc(s.provider || 'Verified NGO')}</p>
          ...
          <div class="result-card-footer">
            ...
            <div style="display:flex;gap:0.5rem;align-items:center">
              <a class="card-link" href="/opportunities" style="font-size:0.82rem">Apply &rarr;</a>
              <button class="card-link remove-bm" data-bm-id="${b.bookmark_id}" style="border:0;background:none;cursor:pointer;font:inherit;color:#c0392b;padding:0;font-size:0.82rem">Remove</button>
            </div>
          </div>
        </article>`;
  ...
  async function initSchoolDashboard() {
    ...
    function renderReqEditor() {
      ...
      reqEditor.innerHTML = reqItems.map((r, i) => `
        <div class="req-row" style="display:flex;gap:0.5rem;align-items:flex-start;margin-bottom:0.5rem">
          <input class="req-label" placeholder="Requirement label" value="${esc(r.item_label)}" style="flex:2;border:1.5px solid rgba(91,15,15,0.12);border-radius:10px;padding:0.65rem 0.85rem;background:#fdfaf8;font:inherit;color:var(--ink)">
          <input class="req-notes" placeholder="Notes (optional)" value="${esc(r.notes || '')}" style="flex:2;border:1.5px solid rgba(91,15,15,0.12);border-radius:10px;padding:0.65rem 0.85rem;background:#fdfaf8;font:inherit;color:var(--ink)">
          <label style="display:flex;align-items:center;gap:0.3rem;font-size:0.82rem;white-space:nowrap;padding-top:0.6rem">
            <input type="checkbox" class="req-required" ${r.is_required ? 'checked' : ''}> Required
          </label>
          <button class="req-remove" data-idx="${i}" style="padding:0.45rem 0.7rem;border:1.5px solid rgba(192,57,43,0.3);border-radius:8px;background:transparent;color:#c0392b;cursor:pointer;font-size:0.82rem;flex-shrink:0;margin-top:0.1rem">&times;</button>
      ...
      if (annEl) {
        annEl.innerHTML = data.announcements.length
          ? data.announcements.map(a => `
              <div class="admin-approve-card" style="flex-direction:column;gap:0.3rem;margin-bottom:0.6rem">
                ...
                <strong style="font-size:0.95rem">${esc(a.title)}</strong>
                <p style="margin:0;font-size:0.84rem;color:var(--muted)">${esc(a.body)}</p>
              </div>`).join('')
        ...
        matEl.innerHTML = data.materials.length
          ? data.materials.map(m => `
              <div class="admin-approve-card" style="flex-direction:column;gap:0.3rem;margin-bottom:0.6rem">
                ...
                <p style="margin:0;font-size:0.84rem;color:var(--muted)">${esc(m.year)} · ${esc(m.type)} · ${esc(m.file_size || 'No file')}</p>
              </div>`).join('')
        ...
        schEl.innerHTML = data.scholarships.length
          ? data.scholarships.map(s => `
              <div class="admin-approve-card" style="flex-direction:column;gap:0.3rem;margin-bottom:0.6rem">
                ...
                <strong style="font-size:0.95rem">${esc(s.title)}</strong>
                <p style="margin:0;font-size:0.84rem;color:var(--muted)">${esc(s.eligibility)}</p>
              </div>`).join('')
  ...
  async function initSchoolDashboard() {
    ...
    function renderReqEditor() {
      ...
      reqEditor.innerHTML = reqItems.map((r, i) => `
        <div class="req-row" style="display:flex;gap:0.5rem;align-items:flex-start;margin-bottom:0.5rem">
          <input class="field-input req-label" placeholder="Requirement" value="${esc(r.item_label)}" style="flex:2">
          <input class="field-input req-notes" placeholder="Notes" value="${esc(r.notes)}" style="flex:2">
          <label style="display:flex;align-items:center;gap:0.3rem;font-size:0.82rem;white-space:nowrap;padding-top:0.6rem">
            <input type="checkbox" class="req-required" ${r.is_required ? 'checked' : ''}> Required
          </label>
          <button class="req-remove" data-idx="${i}" style="padding:0.45rem 0.7rem;border:1.5px solid rgba(192,57,43,0.3);border-radius:8px;background:transparent;color:#c0392b;cursor:pointer;font-size:0.82rem;flex-shrink:0;margin-top:0.1rem">&times;</button>
    ...
    async function loadRecentAnnouncements() {
      ...
      try {
        ...
        el.innerHTML = mine.length
          ? mine.map(a => `<div class="admin-audit-row"><p><strong>${esc(a.title)}</strong> <span class="tag tag-muted" style="font-size:0.72rem">pending</span></p><p class="audit-time">${esc(a.created_at ? a.created_at.slice(0,10) : '')}</p></div>`).join('')
    ...
    async function loadRecentMaterials() {
      ...
      try {
        ...
        el.innerHTML = items.length
          ? items.map(m => `<div class="admin-audit-row"><p><strong>${esc(m.title)}</strong> <span class="tag tag-muted" style="font-size:0.72rem">pending</span></p><p class="audit-time">${esc(m.subject)} &middot; ${esc(m.grade)} &middot; ${esc(m.year)}</p></div>`).join('')
    ...
    async function loadMyScholarships() {
      ...
      try {
        ...
        el.innerHTML = all.length
          ? all.map(s => `<div class="admin-audit-row"><p><strong>${esc(s.title)}</strong> <span class="tag tag-muted" style="font-size:0.72rem">${esc(s.provider || 'Your NGO')}</span></p><p class="audit-time">Deadline: ${esc(s.deadline)}</p></div>`).join('')
          : '<p class="empty-text">No scholarships posted yet.</p>';
      } catch (_) { }
    }
